...
const init = (data) => {
    ...
    let lastWinner;
    let prevWinnersIndex = [];
    let revealedNumbersForLastWinner;
    ...
    do {
        ...
        boards.forEach((board, index) => {
            const wins = isWinner(revealedNumbers, board);

            if (wins && !prevWinnersIndex.includes(index)) {
                prevWinnersIndex.push(index);
                lastWinner = board;
                revealedNumbersForLastWinner = revealedNumbers;
            }
        });
    } while (revealedNumbers.length < numbers.length);

    console.log('Result', getUnmarkedSum(lastWinner, revealedNumbersForLastWinner) * revealedNumbersForLastWinner[revealedNumbersForLastWinner.length - 1]);
